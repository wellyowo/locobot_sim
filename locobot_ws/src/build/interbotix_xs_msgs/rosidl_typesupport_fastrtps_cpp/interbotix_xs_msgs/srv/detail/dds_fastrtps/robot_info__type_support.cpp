  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = interbotix_xs_msgs::srv::RobotInfo_Request;
    is_plain =
      (
...
static size_t _RobotInfo_Request__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_RobotInfo_Request(full_bounded, is_plain, 0);
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = interbotix_xs_msgs::srv::RobotInfo_Response;
    is_plain =
      (
      offsetof(DataType, name) +
      last_member_size
...
static size_t _RobotInfo_Response__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_RobotInfo_Response(full_bounded, is_plain, 0);
