...
REPOSITORY="wellyowo/locobot_2204_test"
TAG="gpu"
...
export DOCKER_BUILDKIT=1 #enable multi-core build
# docker buildx build --rm "$@" -f "${DOCKERFILE_PATH}" --load -t "${IMG}" "${SCRIPT_PATH}" 
docker buildx build --rm "$@" -f "${DOCKERFILE_PATH}" --load -t "${IMG}" --progress=plain --build-arg BUILDKIT_INLINE_CACHE=1 "${SCRIPT_PATH}"
